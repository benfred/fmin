...
export function NelderMead1d(div) {
    ...
    div.select('.function_smooth').on('click', function () {
        obj.graph
            .xDomain([-4.9, 6])
            .yDomain([0, 5])
            .f((x) => Math.log(1 + Math.abs(x) ** (2 + Math.sin(x))));
        obj.redraw();
        obj.initialize([-4.5]);
        div.select('.function_label').html(d3.select(this).html());
    });
    ...
    div.select('.function_noisy').on('click', function () {
        obj.graph
            .xDomain([-5, 5])
            .yDomain([0, 4])
            .f((x) => (2 + Math.sin(50 * x) / 50) * Math.atan(x) * Math.atan(x));
        obj.redraw();
        obj.initialize([-4.5]);
        div.select('.function_label').html(d3.select(this).html());
    });
    ...
    this.expansion = Slider(
        ...
        { format: (d) => `${d.toFixed(1)}x`, initial: 1.0 },
    );
...
NelderMead1d.prototype.increment = function (currentCycle, duration) {
    ...
    this.div
        .select('.iterations')
        .text(
            `Iteration ${this.stateIndex + 1}/${this.states.length}, Loss=${this.states[this.stateIndex].fx.toFixed(5)}`,
        );
