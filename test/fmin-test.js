...
function nearlyEqual(left, right, tolerance = SMALL, message = 'assertNearlyEqual') {
    expect(Math.abs(left - right)).toBeLessThan(tolerance, `${message}: ${left} ~== ${right}`);
}
...
const optimizers = [fmin.nelderMead, fmin.gradientDescent, fmin.gradientDescentLineSearch, fmin.conjugateGradient];
...
test('nelderMead', () => {
    ...
    const solution = fmin.nelderMead(loss, [-3.5, 3.5]);
...
test('conjugateGradientSolve', (test) => {
    ...
    let x = fmin.conjugateGradientSolve(A, b, initial);
